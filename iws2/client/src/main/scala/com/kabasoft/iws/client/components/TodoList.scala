...
import com.kabasoft.iws.gui.macros.Bootstrap.{CommonStyle, Button}
...
object TodoList {
  ...
  private val TodoList = ReactComponentB[TodoListProps]("TodoList")
    .render_P(p => {
      ...
      <.ul(style.listGroup)(p.items.asInstanceOf[Seq[TodoItem]] map renderItem)
    })
    .build

  def apply(items: Seq[IWS], stateChange: TodoItem => Callback, editItem: TodoItem => Callback, deleteItem: TodoItem => Callback) =
