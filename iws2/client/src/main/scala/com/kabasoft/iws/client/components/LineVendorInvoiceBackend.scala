  ...
  class LineVendorInvoiceBackend ($: BackendScope[PropsL[VendorInvoice[LineVendorInvoice], LineVendorInvoice],
    ...
    def render(p:PropsL[VendorInvoice[LineVendorInvoice],LineVendorInvoice], s: State[LineVendorInvoice]) = {
      ...
      def editLine : Seq [TagMod] = List(
          <.div(^.cls :="row",
            ComboBox("Account", itemsx = buildIdNameList(items), defValue = s.item.map(_.account.getOrElse("a0001")).getOrElse("a0001"), updateAccount _, "col-xs-3"),
            buildWItemN[Boolean]("D/C", s.item.map(_.side), true, evt = updateSide, "col-xs-1" ),
            ComboBox("OAccount", buildIdNameList(items),  defValue =s.item.map(_.oaccount.getOrElse("a0007")).getOrElse("a0007"), updateOAccount _, "col-xs-3"),
