...
object Bootstrap {
  ...
  object Button {
    ...
    val component = ReactComponentB[Props]("Button")
      .renderPC((_, p, c) =>
       // <.button(bss.buttonOpt(p.style), p.addStyles, ^.tpe := "button",^.alignSelf := "center" ,^.cls :="form-control btn-danger", ^.verticalAlign:="center" ,^.onClick --> p.onClick, c)
  ...
  object Panel {
    ...
    case class Props(heading: String ="", style: CommonStyle.Value = CommonStyle.default, child:Seq[ReactNode] = Seq.empty[ReactNode])

    val component = ReactComponentB[Props]("Panel")
      .renderPC((_, p, c) =>{

       val  header:TagMod =
         if(!p.heading.isEmpty)
            <.div(bss.panelHeading, p.heading, p.child, ^.paddingTop :=0)
         else  Seq.empty[TagMod]

        <.div(bss.panelOpt(p.style), header,
          <.div(bss.panelBody, c))
       }).build
